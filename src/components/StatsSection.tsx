...
const CircleStat = ({
  ...
  return <motion.div className="circle-stat flex flex-col items-center" initial={{
    ...
    scale: 1.05
      ...
      <div className="relative w-40 h-40 md:w-[200px] md:h-[200px] lg:w-60 lg:h-60" role="progressbar" aria-valuenow={progress} aria-valuemin={0} aria-valuemax={100} aria-label={`${label}: ${value}${suffix}`}>
        <svg className="w-full h-full transform -rotate-90" viewBox={`0 0 ${(radius + strokeWidth) * 2} ${(radius + strokeWidth) * 2}`}>
          ...
          {/* Background circle */}
          <circle cx={radius + strokeWidth} cy={radius + strokeWidth} r={radius} fill="none" stroke="hsl(355 25% 90%)" strokeWidth={strokeWidth} className="md:stroke-[10] lg:stroke-[12]" />
          
          {/* Progress circle */}
          <motion.circle cx={radius + strokeWidth} cy={radius + strokeWidth} r={radius} fill="none" stroke={`url(#${gradientId})`} strokeWidth={strokeWidth} strokeLinecap="round" strokeDasharray={circumference} style={{
