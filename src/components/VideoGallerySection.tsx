...
const VideoGallerySection = () => {
  ...
  return <section className="py-12 md:py-20 bg-cream">
      <div className="container mx-auto px-4">
        ...
        duration: 0.8
          ...
          <div className="hidden md:grid grid-cols-4 gap-4 lg:gap-6">
            ...
            delay: 0.1 + index * 0.05
                ...
                <div className="relative aspect-video rounded-lg overflow-hidden shadow-md hover:shadow-xl transition-all duration-300">
                  <img src={getYoutubeThumbnail(video.youtubeId)} alt={`${video.singer} - ${video.description}`} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" />
          ...
          <div className="md:hidden grid grid-cols-2 gap-4">
            {filteredVideos.map((video, index) => <motion.div key={video.id} initial={{
            opacity: 0,
            y: 20
          }} animate={inView ? {
            opacity: 1,
            ...
            duration: 0.5,
            delay: 0.1 + index * 0.05
          }} className="cursor-pointer" onClick={() => setSelectedVideo(video)}>
                <div className="relative aspect-video rounded-lg overflow-hidden shadow-md">
                  <img src={getYoutubeThumbnail(video.youtubeId)} alt={`${video.singer} - ${video.description}`} className="w-full h-full object-cover" />
                  {/* Play button */}
      ...
      <Dialog open={!!selectedVideo} onOpenChange={() => setSelectedVideo(null)}>
        <DialogContent className="max-w-4xl p-0 bg-black overflow-hidden">
          {selectedVideo && <div className="aspect-video">
              <iframe src={`https://www.youtube.com/embed/${selectedVideo.youtubeId}?autoplay=1${selectedVideo.startTime ? `&start=${selectedVideo.startTime}` : ""}`} title={`${selectedVideo.singer} - ${selectedVideo.description}`} className="w-full h-full" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowFullScreen />
            </div>}
        </DialogContent>
      </Dialog>
    </section>;
